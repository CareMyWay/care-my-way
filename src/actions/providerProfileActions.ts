...
export function transformFormDataToProfile(
  ...
  userId: string,
  profileOwner: string
): ProviderProfileDataDB {
  // Get form sections with proper typing
  const personalContact =
    (formData["personal-contact"] as Record<string, unknown>) || {};
  const address = (formData["address"] as Record<string, unknown>) || {};
  const emergencyContact =
    (formData["emergency-contact"] as Record<string, unknown>) || {};
  const professionalSummary =
    (formData["professional-summary"] as Record<string, unknown>) || {};
  const credentials =
    (formData["credentials"] as Record<string, unknown>) || {};
  ...
  const education = cleanCredentialsData(
    credentials?.education as Array<Record<string, unknown>>
  );
  const certifications = cleanCredentialsData(
    credentials?.certifications as Array<Record<string, unknown>>
  );
  const workExperience = cleanCredentialsData(
    credentials?.workExperience as Array<Record<string, unknown>>
  );
...
function cleanCredentialsData(
  ...
  const cleaned = data
    ...
    .map((entry: Record<string, unknown>) => {
      ...
      if (
        cleaned.documents &&
        Array.isArray(cleaned.documents) &&
        cleaned.documents.length === 0
      ) {
...
function safeParseJSON(
  value: string | number | boolean | object | Array<unknown> | null | undefined
...
export async function updateProviderProfile(
  profileId: string,
  profileData: Partial<ProviderProfileDataDB>
): Promise<ProviderProfileData | null> {
  try {
    ...
    if (profileData.firstNameLower !== undefined)
      updateInput.firstNameLower = profileData.firstNameLower;
    if (profileData.lastNameLower !== undefined)
      updateInput.lastNameLower = profileData.lastNameLower;
    if (profileData.dob !== undefined) updateInput.dob = profileData.dob;
    if (profileData.gender !== undefined)
      updateInput.gender = profileData.gender;
    if (profileData.languages !== undefined)
      updateInput.languages = profileData.languages;
    ...
    if (profileData.preferredContact !== undefined)
      updateInput.preferredContact = profileData.preferredContact;
    if (profileData.profilePhoto !== undefined)
      updateInput.profilePhoto = profileData.profilePhoto;
    if (profileData.address !== undefined)
      updateInput.address = profileData.address;
    if (profileData.city !== undefined) updateInput.city = profileData.city;
    if (profileData.province !== undefined)
      updateInput.province = profileData.province;
    if (profileData.postalCode !== undefined)
      updateInput.postalCode = profileData.postalCode;
    if (profileData.emergencyContactName !== undefined)
      updateInput.emergencyContactName = profileData.emergencyContactName;
    if (profileData.emergencyContactPhone !== undefined)
      updateInput.emergencyContactPhone = profileData.emergencyContactPhone;
    if (profileData.emergencyContactRelationship !== undefined)
      updateInput.emergencyContactRelationship =
        profileData.emergencyContactRelationship;
    if (profileData.profileTitle !== undefined)
      updateInput.profileTitle = profileData.profileTitle;
    ...
    if (profileData.yearExperienceFloat !== undefined)
      updateInput.yearExperienceFloat = profileData.yearExperienceFloat;
    if (profileData.askingRate !== undefined)
      updateInput.askingRate = profileData.askingRate;
    if (profileData.responseTime !== undefined)
      updateInput.responseTime = profileData.responseTime;
    if (profileData.servicesOffered !== undefined)
      updateInput.servicesOffered = profileData.servicesOffered;
    ...
    if (profileData.isProfileComplete !== undefined)
      updateInput.isProfileComplete = profileData.isProfileComplete;
    if (profileData.isPubliclyVisible !== undefined)
      updateInput.isPubliclyVisible = profileData.isPubliclyVisible;

    // Cast to the required input type for the Amplify client
    const result = await client.models.ProviderProfile.update(
      updateInput as Parameters<typeof client.models.ProviderProfile.update>[0]
    );
...
export async function getPublicProviderProfiles(): Promise<
  ProviderProfileData[]
> {
  try {
    const result = await client.models.ProviderProfile.list();
    ...
    if (result.errors) {
      console.error("Error fetching public provider profiles:", result.errors);
      return [];
    }

    // Filter in JavaScript for now (can be optimized later)
    const filteredProfiles = result.data.filter(
      (profile) =>
        profile.isProfileComplete === true && profile.isPubliclyVisible === true
    );

    // Transform each profile and parse JSON strings into arrays
    const transformedProfiles: ProviderProfileData[] = filteredProfiles.map(
      (rawProfile) =>
        ({
          ...rawProfile,
          // Parse JSON strings back into arrays for UI consumption
    ...
    console.error("Failed to get public provider profiles:", error);
    return [];
  }
}

// Transform provider profile data to marketplace format
export function transformProviderForMarketplace(profile: ProviderProfileData) {
  const fullName =
