...
export function Availability({subtitle, selectedAvailability, setSelectedAvailability}:{
  ...
  const handleClick = (e: React.MouseEvent<HTMLDivElement, MouseEvent>): void => {
    if (selectedAvailability.some((a) => a === `${date.toISOString().substring(0, 10)}:${e.currentTarget.id}`)){
      return;
    }
    setSelectedAvailability([... selectedAvailability, `${date.toISOString().substring(0, 10)}:${e.currentTarget.id}`]);
  };
  ...
  const handleDelete = (strInArray: string): void => {
    ...
    setSelectedAvailability(selectedAvailability.filter((a) => a !== strInArray).sort());
  };

  const findDayInSelectedAvailability = (d: number): boolean => {
    return selectedAvailability.some((a) => a.split(":")[0] === `${new Date(year, month, d).toISOString().substring(0, 10)}`);
  };

  const findDayHourInSelectedAvailability = (hh24: string): boolean => {
    const true_hh24 = hh24.substring(hh24.length - 2);
    return selectedAvailability.some((a) => a === `${date.toISOString().substring(0, 10)}:${true_hh24}`);
  };
              ...
              .map((_, index) => {
                ...
                return (
                  <div className={"w-[14.2%] text-center text-zinc-500 h-[40px] flex items-center justify-center cursor-pointer" +
                    ...
                    // isSelected={d === day}
                       onClick={() => {
                         if (year * 10000 + month * 100 + d < today.getFullYear() * 10000 + today.getMonth() * 100 + today.getDate()) {
                           return;
                         }
