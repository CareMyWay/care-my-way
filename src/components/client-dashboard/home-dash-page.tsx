...
export default function HomeDashPage() {
  ...
  // Filter todos based on profile completion
  const todos = profileCompleted
    ? initialTodos.filter((todo) => todo.taskTitle !== "Complete User Profile")
    : initialTodos;
    ...
    const checkProfileCompletion = async () => {
      try {
        ...
        const { data: profiles, errors } =
          await client.models.ClientProfile.list({
            filter: {
              profileOwner: { eq: profileOwner },
            },
          ...
          <Card className="border-1 drop-shadow-sm border-medium-green rounded-lg">
            ...
            <CardContent className="pt-0 space-y-4 max-h-[400px] overflow-y-auto px-10">
              {profileCompleted === null ? (
                ...
                <div className="text-center text-gray-600">
                  No tasks pending. Your profile is complete!
                </div>
              ) : (
                todos.map((task) => (
                  <div
