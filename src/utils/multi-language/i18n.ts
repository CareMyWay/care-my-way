...
import lanSrcJson from "src/utils/multi-language/locales/locales-common.json";
import { initReactI18next } from "react-i18next";
// import {getTranslationFromFrontend} from "@/utils/multi-language/awsTranslateFrontend";
import {getTranslationProxy} from "@/utils/multi-language/awsTranslateProxy";
    ...
    missingKeyHandler: async (lng, ns, key, fallbackValue) => {
      ...
      if (key !== "DUMMY STRING") {
        if (targetLang !== "en" && (!i18n.hasResourceBundle(targetLang, "common") || !i18n.exists(key, { lng: targetLang, ns: "common" }))) {
          ...
          const translated = await getTranslationProxy(fallbackValue || key, targetLang);  // You can also dynamically add this to i18next
